...
begin
 ...
 if combobox1.Text = 'NoThing' then
  begin
   if CreateDir(s+'\bin\data\'+Edit2.Text) then
    memo12.Lines.LoadFromFile(s+'\bin\data\'+Edit2.Text+'\result.out');
  end;
  { else
  if CreateDir(s+'\bin\data\'+Edit2.Text) then;
  begin
  memo12.Lines.LoadFromFile(s+'\bin\result.out');
  memo12.Lines.SaveToFile(s+'\bin\data\'+Edit2.Text+'\result.out');
  end;
  }
